...
window.onload = () => {
    ...
    // Adiciona listener para marcar/desmarcar na lista principal
...
// NOVO: Função para remover um exame da lista de exibição e desmarcar seu checkbox
function removerExameDisplay(exameNome) {
    const checkbox = document.querySelector(`#exames .exame[value="${exameNome}"]`);
    if (checkbox) {
        checkbox.checked = false; // Desmarca o checkbox na lista principal
    }
    atualizarExamesSelecionadosDisplay(); // Atualiza a lista de exibição
}
...
function carregarCadastro(index) {
    ...
    atualizarExamesSelecionadosDisplay(); // Atualiza a lista de exibição após carregar do histórico
...
function limparCampos(showAlert = true) {
    ...
    atualizarExamesSelecionadosDisplay(); // Limpa a lista de exibição também
